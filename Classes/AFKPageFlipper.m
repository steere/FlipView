...
- (void) cleanupFlip {
	...
	if (setNewViewOnCompletion) {
		[self.viewCurrent removeFromSuperview];
		self.viewCurrent = self.viewNew;
		self.viewNew = Nil;
	} else {
	...
	[self.viewCurrent setHidden:FALSE];
...
- (BOOL) doSetCurrentPage:(NSInteger) value {
	...
	self.viewNew = [self.dataSource viewForPage:value inFlipper:self];
...
- (void) setCurrentPage:(NSInteger) value animated:(BOOL) animated {
	...
	pageDifference = labs(value - currentPage);
